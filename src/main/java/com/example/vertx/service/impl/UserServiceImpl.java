...
public class UserServiceImpl extends CommonHttpUtil implements UserService {
    ...
    public void saveUser(RoutingContext routingContext) {
        ...
        JsonArray queryParam = new JsonArray().add(name)
            .add(email);

        jdbcService.saveOrUpdateEntity(sqlQuery, queryParam)
    ...
    public void updateUser(RoutingContext routingContext) {
        ...
        long id = Long.parseLong(routingContext.request()
            .getParam("id"));
    ...
    public void getUserById(RoutingContext routingContext) {
        ...
        String param = routingContext.request()
            .getParam("id");
        if (param == null) {
        ...
        jdbcService.getSingleEntityByParams(sqlQuery, queryParam)
            .setHandler(resultHandler(routingContext, res -> {
                if (!res.isPresent()) {
        ...
        jdbcService.getSingleEntityByParams(sqlQuery, queryParam)
            .setHandler(resultHandler(routingContext, res -> {
                if (!res.isPresent()) {
                    ...
                    List<JsonObject> list = (List<JsonObject>) res.get();
                    List<SystemUser> userList = list.stream()
                        .map(obj -> new SystemUser().getSystemUser(obj))
