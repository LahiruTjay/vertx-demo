...
public class CommonHttpUtil {
	...
	public void successGetResponse(RoutingContext routingContext, Object result) {
		routingContext.response().setStatusCode(200).putHeader("content-type", "application/json")
				.end(Json.encodePrettily(
						new CommonHttpResponse(CommonConstants.CODE_SUCCESSFUL, CommonConstants.CODE_SUCCESSFUL, result)));
	}

	public void serviceUnavailable(RoutingContext routingContext) {
		routingContext.response().setStatusCode(503).end(Json.encodePrettily(
				new CommonHttpResponse(CommonConstants.CODE_FAILED, CommonConstants.HTTP_504_UNAVAILABLE)));
	}
	...
	public void badRequest(RoutingContext routingContext) {
		routingContext.response().setStatusCode(400).end(Json.encodePrettily(
				new CommonHttpResponse(CommonConstants.CODE_FAILED, CommonConstants.HTTP_400_BAD_REQUEST)));
	}
